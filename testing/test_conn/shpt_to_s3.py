            ...
            try:
                ...
                if sf_conn:
                    try:
                        ...
                        cursor.execute(
                            f"CALL DEV_PS_RAW_DB.CORE_REFERENCE.COPY_FROM_S3_TO_RAW_VALIDAITON('{client_name}','{process_name}');"
                        )
                        result = cursor.fetchone()
                        print(f" SP executed for Client={client_name}, Process={process_name}, Result={result}")
                    except Exception as e:
                        print(f" Failed to call Snowflake SP for Client={client_name}, Process={process_name}: {e}")
                    finally:
                        cursor.close()
